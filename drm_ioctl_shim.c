...
static struct { uint32_t prime_fd; buffer_handle_t gralloc; } gmap[GMAP_MAX];
static int gmap_n = 0;
static struct { uint32_t gem, fb_id; } fmap[GMAP_MAX];
static int fmap_n = 0;
...
void drm_shim_register_bo(uint32_t prime_fd, buffer_handle_t gralloc) {
    ...
    if (gmap_n<GMAP_MAX) { gmap[gmap_n].prime_fd=prime_fd; gmap[gmap_n++].gralloc=gralloc; }
}
    ...
    if (res&&res->count_crtcs>0&&res->count_connectors>0) {
        ...
        if (conn&&conn->count_modes>0) {
            ...
            LOG("saved mode from connector: %dx%d@%dHz clock=%d",
                ...
                saved_mode.vrefresh, saved_mode.clock);
            /* Use the DRM connector's preferred mode refresh rate.
             * The connector reports the panel's raw capability which may be
             * higher than what HWC2 will actually use. Find the mode with
             * the highest refresh rate that the connector reports as preferred,
             * or fall back to the first mode. This avoids hardcoding a specific
             * Hz value and works across different devices. */
            for (int m = 0; m < conn->count_modes; m++) {
                struct drm_mode_modeinfo *mode =
                    (struct drm_mode_modeinfo*)&conn->modes[m];
                if (mode->type & DRM_MODE_TYPE_PREFERRED) {
                    saved_mode = *mode;
                    LOG("using preferred mode: %dx%d@%dHz clock=%d",
                        saved_mode.hdisplay, saved_mode.vdisplay,
                        saved_mode.vrefresh, saved_mode.clock);
                    break;
                }
...
int drmModeAddFB2WithModifiers(int fd,uint32_t w,uint32_t h,uint32_t fmt,
    ...
    if (fmap_n<GMAP_MAX) { fmap[fmap_n].gem=handles[0]; fmap[fmap_n++].fb_id=id; }
    return 0;
}
...
int ioctl(int fd, unsigned long request, ...) {
    ...
    if (nr==0xb8||nr==0xb9) {
        ...
        fb[6]=id;
        if (fmap_n<GMAP_MAX) { fmap[fmap_n].gem=gem; fmap[fmap_n++].fb_id=id; }
        ret=0;
    } else if (nr==0xa7) {
        /* GETCONNECTOR - fix up reported modes to use correct refresh rate.
         * The kernel reports raw panel capability (e.g. 303Hz) but HWC2
         * only supports the actual refresh rate. We find the preferred mode
         * and replace all reported modes with just that one. */
        ret = real_ioctl(fd, request, arg);
        if (ret == 0) {
            struct drm_mode_get_connector *conn_arg = arg;
            if (conn_arg->modes_ptr && conn_arg->count_modes > 0) {
                struct drm_mode_modeinfo *modes =
                    (struct drm_mode_modeinfo *)(uintptr_t)conn_arg->modes_ptr;
                /* Find preferred mode */
                struct drm_mode_modeinfo *preferred = &modes[0];
                for (uint32_t m = 0; m < conn_arg->count_modes; m++) {
                    if (modes[m].type & DRM_MODE_TYPE_PREFERRED) {
                        preferred = &modes[m];
                        break;
                    }
                }
                /* Fix the clock to match vrefresh so mutter calculates
                 * the correct refresh rate. Mutter uses:
                 *   refresh = clock*1000 / (htotal*vtotal)
                 * so we recalculate clock from the actual vrefresh. */
                struct drm_mode_modeinfo fixed = *preferred;
                if (fixed.vrefresh > 0 && fixed.htotal > 0 && fixed.vtotal > 0) {
                    uint32_t correct_clock = (uint32_t)(
                        (uint64_t)fixed.htotal * fixed.vtotal *
                        fixed.vrefresh / 1000);
                    if (correct_clock != fixed.clock) {
                        g_warning("drm_shim: fixing clock %u->%u for %dHz",
                            fixed.clock, correct_clock, fixed.vrefresh);
                        fixed.clock = correct_clock;
                    }
                }
                /* Save it for later use */
                if (saved_mode.hdisplay == 0)
                    saved_mode = fixed;
                /* Replace all modes with just the fixed preferred one */
                modes[0] = fixed;
                conn_arg->count_modes = 1;
                g_warning("drm_shim: GETCONNECTOR fixed modes to %dx%d@%dHz clock=%u",
                    fixed.hdisplay, fixed.vdisplay,
                    fixed.vrefresh, fixed.clock);
            }
        }
    } else if (nr==0xaf) {
        ret=0;
    } else if (nr==0xa1) {
        /* GETCRTC - fix up the mode to report correct refresh rate */
        ret = real_ioctl(fd, request, arg);
        if (ret == 0 && saved_mode.hdisplay > 0) {
            struct drm_mode_crtc *crtc = arg;
            if (crtc->mode_valid) {
                struct drm_mode_modeinfo *m =
                    (struct drm_mode_modeinfo *)&crtc->mode;
                if (m->vrefresh != saved_mode.vrefresh) {
                    g_warning("drm_shim: GETCRTC fixing %dHz -> %dHz",
                        m->vrefresh, saved_mode.vrefresh);
                    *m = saved_mode;
                }
            }
        }
